...
aPlayer.Base = new Class({
    ...
    options: {
		...
		snds: [
			...
			{id: 'sndGood', url: '../main/audio/truing1.mp3'}
		],
...
aPlayer.Base.implement(new Events, new Options);
...
aPlayer.speakers = aPlayer.Base.extend ({
	options: {
		sounds: 0,
		adjSpeakers: false,
		prefixFile: 'snd',
	...
	eventControl: function (speaker) {
		...
		//(this.speakers).each (function (speaker, iS){
			...
			if (this.options.cssName) speaker.addClass (this.options.cssName + '-ini');
	...
	finishPlay: function (speaker) {
		if (this.options.cssName) speaker.removeClass (this.options.cssName + '-play');
	},
    ...
    adjustSpeakers: function(){
		...
		(this.speakers).each(function(speaker, iS){
			...
			speaker.setPosition(this.options.adjSpeakers[iS]);
		}, this);
	},
 ...
 */
aPlayer.rePro = aPlayer.Base.extend ({
...
aPlayer.karaoke = aPlayer.rePro.extend ({
			...
			if (this.timeKnobAnt != timeKnob) {
				...
				if (verse.ind != this.indVerseAnt) {
					...
					if (stanza.ind != this.indStanzaAnt) {
						...
						//this.stanzaUp (stanza, (this.indStanzaAnt == -1) ? this.stanzas[0] : this.stanzas[this.indStanzaAnt]);
	...
	stanzaUp: function (stanza, stanzaAnt) {
		...
		if (stanzaAnt) {
			stanzaAnt.setStyles(this.options.stylesStanzaDown);
		}
		else if (stanza.ind != 0) this.stanzas[0].setStyles(this.options.stylesStanzaDown);
