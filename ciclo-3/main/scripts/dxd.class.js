	...
	dropComplete: function(drag, drags){
		...
		if (((drags.overed).qttDrops <= (drags.overed).numDrops) || this.options.multiDrop == 'infinity') {
			...
			if (this.fncTrueDrop(drag, (drags.overed))) {
				...
				if (this.options.textInDrop) (drags.overed).setText((drags.overed).text);
			};
		}
		else {
            // SendToBack ?
...
DxD.Film = DxD.Base.extend({
	...
	setIniFilm: function(){
		...
		this.drags.each(function(drag, iDrag){
            ...
            if (this.options.bgOffsetDrag) drag.setStyle('background-position', (this.options.bgOffsetDrag * iDrag).toString () + 'px top');
        }, this);
...
DxD.Text = DxD.Base.extend({
	...
	setIniText: function (drags, drops, options) {
		...
		drops.each (function (drop, iD){
			this.newDrops.extend(drops[iD].getElements('.' + this.options.hiddenClass));
		}, this);
				...
				destripe.each(function(sword, iS){
					...
					if ((sword.length)) {
						...
						if (condSwordTag) {
							...
							if (!optsSword) stringHTML = stringHTML + '<span class="' + this.options.hiddenClass + '">' + line.arrTxT[j] + '</span></span>';
							else {
								var maxSword = '';
								var maxLengthId = 0;
...
//			if (stringHTML.length) console.log ('stringHTML: ' + stringHTML, + 'longitud: ' + stringHTML.length);
	...
	fncTrueDrop: function(drag, drop) {
		...
		var fncControl = function (textoDrag, textoDrop) {
			...
			if ((textoDrag == dropSword) && condSword) {
				return true
			}
			else {
				return false;
