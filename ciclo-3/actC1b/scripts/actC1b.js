...
window.onDomReady(function(){
	...
	var setCircular = function (obj, options) {
		...
		obj.each (function (el, iE) {
			...
			el.addClass('actC1b-Ini');

			// Eventos
			el.addEvents ({
				'mouseenter': function () {el.addClass ('actC1b-enter')},
				'mouseleave': function () {el.removeClass ('actC1b-enter')},
				'mousedown': function () {

					if (obj.petAct == iE) {
					el.removeClass('actC1b-Act');
						obj.petAct = -1;
					}
					else {
						if (obj.petAct != -1) obj[obj.petAct].removeClass('actC1b-Act');
						el.addClass('actC1b-Act');
						obj.petAct = iE;
					};
	...
	// objeto actC1b tipo DxD.Base
	var actC1b = new DxD.Text (sword, drops, {
		container: $('content'),
		sendToBack: 'ini',
		adjDrags: [{x: 425, y: 0}],
		cssClass: 'sword'
	...
	actC1b.addEvent ('trueDrop', function (drag, drop){
		drag.setText ('?');
